...
# Start the core stack (--profile local includes Garage S3 for local dev)
...
The bootstrap script auto-detects **Garage** (local dev) vs **AWS S3** (production) mode:

**Local dev (Garage):**
1. Generates cryptographic secrets for Garage
2. Creates the `lakehouse` S3 bucket and access key
3. Writes `.env` with `S3_*` and `GARAGE_*` credentials
4. Bootstraps LakeKeeper (accepts ToS, creates warehouse)

**AWS S3 (production):**
1. Reads `S3_*` variables from `.env` (set by CI/CD or manually)
2. Skips Garage setup entirely
3. Bootstraps LakeKeeper with S3 warehouse config

## Setting Up `.env`

The `.env` file is **auto-generated by `bootstrap.sh`** for local dev, or written by `deploy/deploy.sh` for production. It is excluded from git.

**Local dev** (auto-generated by `bootstrap.sh`):
```bash
S3_ACCESS_KEY_ID=GKxxxxxxxxxxxxxxxxxxxxxxxx
...
**AWS S3 production** (set manually or via GitLab CI/CD variables):
```bash
S3_ACCESS_KEY_ID=AKIA...
S3_SECRET_ACCESS_KEY=wJal...
S3_ENDPOINT=s3.us-east-1.amazonaws.com
S3_REGION=us-east-1
S3_BUCKET=terminus-lakehouse
S3_URL_STYLE=vhost
S3_USE_SSL=true
S3_PATH_STYLE=false
S3_FLAVOR=aws
...
See [`deploy/env.template`](../deploy/env.template) for all available variables.

**For EC2 deployment**: `PYGEOAPI_SERVER_URL=auto` combined with nginx's `sub_filter` dynamically rewrites all OGC API links to match the request hostname.
...
All services use generic `S3_*` environment variables. To switch from Garage to AWS S3:

1. Set `S3_*` variables in `.env` (see template above) or GitLab CI/CD variables
2. Start **without** `--profile local` (Garage won't start)
3. Run `./bootstrap.sh` — it detects S3 mode and bootstraps LakeKeeper accordingly
...
Or use the CI/CD pipeline: push to `main` with GitLab variables configured.

Your Iceberg tables, schemas, and SQL all stay the same — only the storage backend changes.

## Resource Usage

| Profile | RAM | Services |
|---------|-----|----------|
| Core (default) | ~3.5GB | LakeKeeper, Postgres, DuckDB, MCP, API, pygeoapi, geoservices, webmap |
| Local (`--profile local`) | ~4GB | Adds Garage S3 (local dev only) |
